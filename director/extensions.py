...
class CeleryWorkflow:
    ...
    def init_app(self, app):
        ...
        if self.app.config["DIRECTOR_HOME_WORKFLOW"]:
            self.path = Path(self.app.config["DIRECTOR_HOME_WORKFLOW"]).resolve()
        else:
    ...
    def add_workflow(self, name, conf):
        self.workflows[name] = conf
        # with open(self.path, "w") as f:
        #     yaml.dump(self.workflows, f)
    
    def update_workflow(self, name, conf):
        self.workflows[name].update(conf)
        # with open(self.path, "w") as f:
        #     yaml.dump(self.workflows, f)
    ...
    def import_user_tasks(self):
        ...
        if self.app.config["DIRECTOR_HOME_TASKS"]:
            folder = Path(self.app.config["DIRECTOR_HOME_TASKS"]).resolve()
