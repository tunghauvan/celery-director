...
def create_definition():
    ...
    fullname = f"{data['project']}.{data['name']}"
    if fullname in cel_workflows.workflows:
        abort(400, f"Workflow {fullname} already exists")
    cel_workflows.add_workflow(fullname, data)
    return jsonify({"fullname": fullname, **data}), 201
